...
function datesBetween(fromDate, toDate) {
  ...
  for (
    ...
    current <= new Date(`${toDate}T00:00:00Z`);
  ...
  try {
    ...
    if (!validDate(fromDate) || !validDate(toDate) || fromDate > toDate)
      return response(
        res,
        400,
        "Leave dates must be valid and fromDate must not be after toDate",
      );
    if (!["sick", "casual", "other"].includes(type) || !reason?.trim())
      return response(res, 400, "A valid leave type and reason are required");
...
export async function listStudentLeaves(req, res) {
  try {
    ...
    const leaves = await LeaveRequest.find({ studentId: student._id })
      .select("fromDate toDate type reason status teacherRemarks createdAt")
      .sort({ createdAt: -1 });
...
export async function listTeacherLeaves(req, res) {
  try {
    ...
    if (!(await teacherClass(classSectionId, req.user.userId)))
      return response(
        res,
        403,
        "You can only review leave for your assigned class",
      );
    if (!["pending", "approved", "rejected"].includes(status))
      return response(res, 400, "Invalid leave status");
    const leaves = await LeaveRequest.find({ classSectionId, status })
      .populate({
        path: "studentId",
    ...
    return response(
      ...
      leaves.map((leave) => ({
        ...leave.toObject(),
        requestedDays: datesBetween(leave.fromDate, leave.toDate).length,
      })),
    );
  } catch (error) {
    return response(res, 500, "Unable to load leave requests", error.message);
...
export async function reviewLeave(req, res) {
  ...
  try {
    ...
    if (!(await teacherClass(leave.classSectionId, req.user.userId)))
      return response(
        res,
        403,
        "You can only review leave for your assigned class",
    ...
    if (leave.status === status)
      return response(res, 409, "Leave request is already in that status");
    if (leave.status === "rejected" && status !== "approved")
      return response(res, 409, "This leave request cannot be reverted");

    const requestedDays = datesBetween(leave.fromDate, leave.toDate).length;
    const balanceFilter = {
      studentId: leave.studentId,
      leaveType: leave.type,
      academicYear: academicYearFor(leave.fromDate),
    };
    ...
    await session.withTransaction(async () => {
      if (approving) {
        const balance =
          await LeaveBalance.findOne(balanceFilter).session(session);
        if (!balance)
          throw Object.assign(new Error("Leave balance is not configured"), {
            status: 400,
            remaining: 0,
          });
        if (!override && balance.used + requestedDays > balance.totalAllotted)
          throw Object.assign(new Error("Leave exceeds available balance"), {
            status: 400,
            remaining: balance.totalAllotted - balance.used,
          });
        const updated = await LeaveBalance.findOneAndUpdate(
          override
            ...
            : {
                ...balanceFilter,
                $expr: {
                  $lte: [{ $add: ["$used", requestedDays] }, "$totalAllotted"],
                },
              },
        ...
        const updated = await LeaveBalance.findOneAndUpdate(
          { ...balanceFilter, used: { $gte: requestedDays } },
          { $inc: { used: -requestedDays } },
          { new: true, session },
        );
        if (!updated)
          throw Object.assign(
            new Error("Leave balance cannot become negative"),
            { status: 400 },
          );
      }
      ...
      if (status === "approved") {
        ...
        const existingByDate = new Map(
          existing.map((record) => [record.date, record.status]),
        );
        const operations = dates
          .filter((date) => existingByDate.get(date) !== "present")
          .map((date) => ({
            updateOne: {
              filter: { studentId: leave.studentId, date, periodNumber: null },
              update: {
                $set: {
                  classSectionId: leave.classSectionId,
                  periodNumber: null,
                  status: "leave",
    ...
    return response(
      res,
      error.status || 500,
      error.status ? error.message : "Unable to review leave request",
  ...
  try {
    ...
    if (!student)
      return response(
        res,
        403,
        "You can only view leave for your linked children",
      );
    const leaves = await LeaveRequest.find({ studentId: student._id })
      .select("fromDate toDate type reason status teacherRemarks createdAt")
      .sort({ createdAt: -1 });
