...
export async function createClass(req, res) {
  try {
    ...
    if (!grade || !section || !classTeacherId) return response(res, 400, 'Grade, section, and class teacher are required');
    if (!(await getTeacher(classTeacherId))) return response(res, 400, 'classTeacherId must reference a teacher');

    const classSection = await ClassSection.create({ grade, section, classTeacherId });
    await classSection.populate('classTeacherId', 'name email');
    return response(res, 201, 'Class-section created', classSection);
  } catch (error) {
    if (error.code === 11000) return response(res, 409, 'That grade and section already exists');
...
export async function listClasses(req, res) {
  try {
    const classes = await ClassSection.find().populate('classTeacherId', 'name email').sort({ grade: 1, section: 1 });
...
export async function updateClass(req, res) {
  try {
    ...
    const classSection = await ClassSection.findByIdAndUpdate(req.params.id, updates, { new: true, runValidators: true })
...
async function createAccount({ name, email, password, role }) {
  ...
  if (await User.exists({ email: normalizedEmail })) throw Object.assign(new Error('A user with that email already exists'), { status: 409 });
  return User.create({ name, email: normalizedEmail, password: await bcrypt.hash(password, 12), role });
}
...
export async function createTeacher(req, res) {
  try {
    ...
    return response(res, error.status || 500, error.status ? error.message : 'Unable to create teacher', error.status ? undefined : error.message);
  }
}
...
export async function createStudent(req, res) {
  ...
  try {
    ...
    if (!rollNo || !classSectionId) return response(res, 400, 'Roll number and class-section are required');
    ...
    const student = await Student.create({ userId: user._id, classSectionId, rollNo });
    await student.populate([{ path: 'userId', select: 'name email' }, { path: 'classSectionId' }]);
    return response(res, 201, 'Student created', student);
  } catch (error) {
    if (user?._id) await User.findByIdAndDelete(user._id);
    if (error.code === 11000) return response(res, 409, 'That email or roll number already exists');
    return response(res, error.status || 500, error.status ? error.message : 'Unable to create student', error.status ? undefined : error.message);
  }
}
...
export async function listStudents(req, res) {
  try {
    ...
    if (req.query.classSectionId) {
      if (!validId(req.query.classSectionId)) return response(res, 400, 'Invalid classSectionId');
...
function studentLookup(value) {
  return value.includes('@') ? { 'userId.email': value.toLowerCase().trim() } : { rollNo: value.trim() };
}
...
export async function createParent(req, res) {
  ...
  try {
    ...
    if (!Array.isArray(students) || students.length === 0) return response(res, 400, 'At least one student roll number or email is required');
    ...
    const matches = students.map((value) => {
      ...
      return studentDocs.find((student) => lookup.rollNo ? student.rollNo === lookup.rollNo : student.userId?.email === lookup['userId.email']);
    });
    if (matches.some((student) => !student)) {
      await User.findByIdAndDelete(user._id);
      return response(res, 404, 'One or more students could not be found');
    }
    await Student.updateMany({ _id: { $in: matches.map((student) => student._id) } }, { $addToSet: { parentIds: user._id } });
    return response(res, 201, 'Parent created and linked to students', publicUser(user));
  } catch (error) {
    if (user?._id) await User.findByIdAndDelete(user._id);
    return response(res, error.status || 500, error.status ? error.message : 'Unable to create parent', error.status ? undefined : error.message);
  }
}