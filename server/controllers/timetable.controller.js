  ...
  return (
    ...
    periods.every(
      (period) =>
        ...
        validId(period.subjectId) &&
        validId(period.teacherId),
    )
  );
}
...
export async function createTimetable(req, res) {
  try {
    ...
    if (
      !["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].includes(dayOfWeek) ||
      !validPeriods(periods)
    )
      return response(res, 400, "A valid day and periods are required");
    ...
    if (
      (await User.countDocuments({
        _id: { $in: teacherIds },
        role: "teacher",
      })) !== new Set(teacherIds).size
    )
      return response(
        res,
        400,
        "Every timetable teacher must be a teacher user",
      );
    ...
    return response(
      ...
      200,
      "Timetable saved",
      await populateTimetable(Timetable.findById(timetable._id)),
    );
  } catch (error) {
    return response(
      res,
      ...
      error.code === 11000
        ? "Timetable already exists for this class and day"
        : "Unable to save timetable",
      error.code === 11000 ? undefined : error.message,
    );
  }
}
...
export async function listAdminTimetable(req, res) {
  try {
    if (!validId(req.query.classSectionId))
      return response(res, 400, "A valid classSectionId is required");
    return response(
      res,
      200,
      "Timetable loaded",
      await populateTimetable(
        Timetable.find({ classSectionId: req.query.classSectionId }),
      ),
    );
  } catch (error) {
    return response(res, 500, "Unable to load timetable", error.message);
  }
}
...
export async function listTeacherTimetable(req, res) {
  try {
    ...
    return response(
      ...
      await populateTimetable(
        Timetable.find({
          classSectionId: { $in: classes.map((item) => item._id) },
        }),
      ),
    );
  } catch (error) {
...
export async function listStudentTimetable(req, res) {
  try {
    ...
    return response(
      ...
      "Timetable loaded",
      await populateTimetable(
        Timetable.find({ classSectionId: student.classSectionId }),
      ),
    );
  } catch (error) {
    return response(res, 500, "Unable to load timetable", error.message);
  ...
  try {
    ...
    if (
      ...
      return response(
        res,
        403,
        "You can only edit timetables for your assigned classes",
      );
    if (
      !["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].includes(dayOfWeek) ||
      !Array.isArray(periods) ||
      ...
      return response(
        res,
        400,
        "A valid day and at least one period are required",
      );
    ...
    for (const period of periods) {
      if (!Number.isInteger(period.periodNumber) || period.periodNumber < 1)
        return response(
          res,
          400,
          "Every period needs a positive period number",
        );
      let subjectId = period.subjectId;
      if (!validId(subjectId)) {
        if (!period.subjectName?.trim() || !period.subjectCode?.trim())
          return response(res, 400, "Each new subject needs a name and code");
        const subject = await Subject.findOneAndUpdate(
          { code: period.subjectCode.trim().toUpperCase() },
          {
            $set: { name: period.subjectName.trim() },
            $setOnInsert: { code: period.subjectCode.trim().toUpperCase() },
          },
          { new: true, upsert: true, runValidators: true },
        );
    ...
    if (
      new Set(normalizedPeriods.map((period) => period.periodNumber)).size !==
      normalizedPeriods.length
    )
      return response(res, 400, "Period numbers must be unique for a day");
    const timetable = await Timetable.findOneAndUpdate(
      { classSectionId, dayOfWeek },
