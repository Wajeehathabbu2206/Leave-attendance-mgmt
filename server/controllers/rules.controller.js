...
export async function getTeacherAttendanceAlerts(req, res) {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.query.classSectionId) || !(await ClassSection.exists({ _id: req.query.classSectionId, classTeacherId: req.user.userId }))) return response(res, 403, 'You can only view alerts for your assigned class');
    const students = await Student.find({ classSectionId: req.query.classSectionId }).populate('userId', 'name').select('userId');
    const warnings = (await Promise.all(students.map(async (student) => ({ studentId: student._id, name: student.userId?.name, warnings: await getAttendanceAlerts(student._id) })))).filter((item) => item.warnings.length);
    return response(res, 200, 'Class attendance alerts loaded', warnings);
  } catch (error) { return response(res, 500, 'Unable to load class attendance alerts', error.message); }
}