...
export async function teacherDashboard(req, res) {
  try {
    const classSection = await assignedClass(
      req.query.classSectionId,
      req.user.userId,
    );
    if (!classSection)
      return response(
        res,
        403,
        "You can only view your assigned class dashboard",
      );
    const date = today();
    ...
    return response(res, 200, "Teacher dashboard loaded", {
      date,
      attendanceSummary,
      totalStudents,
...
export async function adminDashboard(req, res) {
  try {
    ...
    return response(res, 200, "Admin dashboard loaded", {
      totalStudents,
      totalTeachers,
      totalClasses,
      totalMarked,
      attendanceSummary,
      overallAttendancePercentage: totalMarked
        ? Number(((attended / totalMarked) * 100).toFixed(2))
        : 0,
      pendingLeaveRequests,
    });
  } catch (error) {
  ...
  try {
    ...
    if (!classSection)
      return response(
        res,
        403,
        "You can only export reports for your assigned class",
      );
    ...
    records.forEach((record) => {
      ...
      byStudent.get(key).push(record.status);
    });
    ...
    const rows = students.map((student) => {
      ...
      const percentage = statuses.length
        ? ((counts.present + counts.late) / statuses.length) * 100
        : 0;
      return [
        student.userId?.name,
        student.rollNo,
    ...
    const csv = [
      "Student Name,Roll No,Attendance %,Present Days,Absent Days,Late Days,Leave Days",
      ...rows,
    ].join("\n");
