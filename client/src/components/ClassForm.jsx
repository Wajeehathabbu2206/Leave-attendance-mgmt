...
export default function ClassForm({ teachers, onSubmit, busy }) {
  ...
  return (
    <form className="grid gap-4 md:grid-cols-4" onSubmit={submit}>
      <input className="field" name="grade" onChange={update} placeholder="Grade (e.g. 10)" required value={form.grade} />
      <input className="field" name="section" onChange={update} placeholder="Section (e.g. A)" required value={form.section} />
      <select className="field" name="classTeacherId" onChange={update} required value={form.classTeacherId}>
